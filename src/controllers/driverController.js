  ...
  try {
    ...
    const [performanceRows] = await db.execute(
      ...
      SELECT
        COALESCE(SUM(
          CASE
            WHEN DATE(s.created_at) = CURRENT_DATE
        ...
        COALESCE(SUM(
          CASE
            WHEN (EXTRACT(YEAR FROM s.created_at) = EXTRACT(YEAR FROM CURRENT_DATE) AND EXTRACT(WEEK FROM s.created_at) = EXTRACT(WEEK FROM CURRENT_DATE))
            THEN COALESCE(si.delivered_qty, si.quantity, 0)
            ELSE 0
          END
