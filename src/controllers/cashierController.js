...
const ensureCashierClosingPettyCashColumn = async (connection) => {
  const [cols] = await connection.query(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = current_schema() AND TABLE_NAME = 'cashier_closings' AND COLUMN_NAME = 'petty_cash'`
  );

  if (!cols.length) {
    await connection.query(
      `ALTER TABLE cashier_closings ADD COLUMN petty_cash DECIMAL(12,2) NOT NULL DEFAULT 0.00 `
    );
  }
};
...
// The window counts only activity  point, so:
...
const ensureSettlementSettledAtColumn = async (connection) => {
  const [cols] = await connection.query(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = current_schema() AND TABLE_NAME = 'settlement_history' AND COLUMN_NAME = 'settled_at'`
  );
  if (!cols.length) {
    await connection.query(
      `ALTER TABLE settlement_history ADD COLUMN settled_at DATETIME NULL `
    );
  }
};
...
const ensureExpensePaymentColumns = async (connection) => {
  const requiredColumns = {
    payment_mode: "ALTER TABLE expenses ADD COLUMN payment_mode enum('CASH','UPI','CARD','BANK_TRANSFER') DEFAULT NULL ",
    payment_reference: "ALTER TABLE expenses ADD COLUMN payment_reference varchar(120) DEFAULT NULL ",
  };
...
const ensureOfficeExpensePaymentColumns = async (connection) => {
  const requiredColumns = {
    payment_mode: "ALTER TABLE office_expenses ADD COLUMN payment_mode enum('CASH','UPI','CARD','BANK_TRANSFER') DEFAULT NULL ",
    payment_reference: "ALTER TABLE office_expenses ADD COLUMN payment_reference varchar(120) DEFAULT NULL ",
  };
...
const makeRangeDateCond = (startDate, endDate) => (dateExpr) => {
  ...
  return { sql: `AND DATE(${dateExpr}) = CURRENT_DATE`, params: [] };
};

// Running-day window: everything  given anchor (see getCurrentDayAnchor
...
const getCashLedger = async (connection, makeDateCond) => {
  ...
  const [expModeCol] = await connection.query(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = current_schema() AND TABLE_NAME = 'expenses' AND COLUMN_NAME = 'payment_mode'`
  );
  ...
  const [oeStatusCol] = await connection.query(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = current_schema() AND TABLE_NAME = 'office_expenses' AND COLUMN_NAME = 'status'`
  );

  const [oeModeCol] = await connection.query(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = current_schema() AND TABLE_NAME = 'office_expenses' AND COLUMN_NAME = 'payment_mode'`
  );
...
const ensureNewConnectionCashierTables = async (connection) => {
  await connection.query(`
    CREATE TABLE IF NOT EXISTS customer_new_connection_products (
      ...
      status VARCHAR(100) CHECK (status IN ('PENDING','APPROVED')) NOT NULL DEFAULT 'PENDING',
  ...
  const requiredColumns = {
    product_id: "ALTER TABLE customer_new_connections ADD COLUMN product_id INT DEFAULT NULL ",
    id_proof_url: "ALTER TABLE customer_new_connections ADD COLUMN id_proof_url VARCHAR(500) DEFAULT NULL ",
    payment_mode: "ALTER TABLE customer_new_connections ADD COLUMN payment_mode enum('CASH','UPI','CARD','BANK_TRANSFER') DEFAULT NULL ",
    payment_reference_id: "ALTER TABLE customer_new_connections ADD COLUMN payment_reference_id varchar(120) DEFAULT NULL ",
    cashier_remarks: "ALTER TABLE customer_new_connections ADD COLUMN cashier_remarks text ",
    paid_at: "ALTER TABLE customer_new_connections ADD COLUMN paid_at timestamp NULL DEFAULT NULL ",
  };
...
export const getCashierDashboard = async (req, res) => {
  ...
  try {
    ...
    if (hasRange && startDate === endDate) {
      const [todayCheck] = await connection.query('SELECT (? = CURRENT_DATE) AS isToday', [startDate]);
...
const ensureTransferVoucherPaymentColumns = async (connection) => {
  const requiredColumns = {
    payment_mode: "ALTER TABLE customer_connection_transfers ADD COLUMN payment_mode enum('CASH','UPI','CARD','BANK_TRANSFER') DEFAULT NULL ",
    payment_reference_id: "ALTER TABLE customer_connection_transfers ADD COLUMN payment_reference_id varchar(120) DEFAULT NULL ",
    cashier_remarks: "ALTER TABLE customer_connection_transfers ADD COLUMN cashier_remarks text ",
  };
  ...
  try {
    ...
    const [rows] = await connection.query(
      ...
      SELECT
        ...
        cnc.cashier_remarks,
        cnc.payment_status,
        TO_CHAR(cnc.created_at, 'YYYY-MM-DD HH24:MI:SS') AS created_at,
        TO_CHAR(cnc.paid_at, 'YYYY-MM-DD HH24:MI:SS') AS paid_at,
  ...
  try {
    const [summaryRows] = await connection.query(
      ...
      SELECT
        ...
        COALESCE(SUM(CASE WHEN e.status = 'APPROVED' AND DATE(e.created_at) = CURRENT_DATE THEN e.amount ELSE 0 END), 0) AS approvedToday
      FROM expenses e
...
const ensureCashierReceiptsTable = async (connection) => {
  await connection.query(`
    CREATE TABLE IF NOT EXISTS cashier_receipts (
      id INT GENERATED BY DEFAULT AS IDENTITY,
      cashier_id INT NULL,
      receipt_type VARCHAR(100) CHECK (status IN ('ADVANCE','DUE_COLLECTION','OTHER')) NOT NULL,
      amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
      description VARCHAR(255) NULL,
      payment_mode VARCHAR(100) CHECK (status IN ('CASH','UPI','BANK_TRANSFER','CARD')) NOT NULL DEFAULT 'CASH',
